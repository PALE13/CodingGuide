...
<img src="https://palepics.oss-cn-guangzhou.aliyuncs.com/img/image-20240227161627341.png" alt="image-20240227161627341" style="zoom: 67%;" />
...
class Printer {
    private int turn = 0; // 控制轮到哪个线程打印

    public synchronized void print(int id , String s) {
        while (turn != id) {
            try {
                wait(); // 当前线程不是轮到的线程，进入等待状态
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }
...
当一个线程调用另一个线程的 `join()` 方法时，它会被阻塞，直到被调用的线程执行完毕或者超时。在其他线程调用的线程执行完毕之前，调用 `join()` 方法的线程将一直保持阻塞状态。
