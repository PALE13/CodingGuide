...
3. 延迟双删，**可以保证数据最终一致性，但是会出现少量不一致性**
4. 「先更新数据库， 再删除缓存」其实是两个操作，如果在删除缓存（第二个操作）的时候失败了，依然会出现数据不一致的情况，如何保证「先更新数据库 ，再删除缓存」这两个操作能执行成功？有两种方法：a. 使用「消息队列来重试缓存的删除」。b. 「订阅 MySQL binlog 再操作缓存」。
